...
0,c=function(a,d,e){for(var f=b.indexToCoords(a),f=[{x:f.x,y:f.y-1},{x:f.x,y:f.y+1},{x:f.x-1,y:f.y},{x:f.x+1,y:f.y}],h=0;h<f.length;h++){var m=f[h],m=b.coordsToIndex(m.x,m.y);0<=m&&null!==b.state[m]&&b.state[m]==d&&-1==e.indexOf(m)&&(-1==e.indexOf(a)&&e.push(a),e.push(m),c(m,d,e))}},d=b.state.length-1;0<=d;d--)if(null!==b.state[d]){var e=[];c(d,b.state[d],e);if(5<=e.length){for(var f=0;f<e.length;f++)b.state[e[f]]=null;a+=e.length}}return a};this.update=function(a,c){b.addPlayerBlocks(c.player);for(var d=
null,e=0;0!==d;)e++,b.dropFlyingBlocks(),d=this.removeConnectingBlocks(),c.score+=100*d*e;c.updateHud();c.blockLayer.removeAllChildren();for(e=0;e<b.state.length;e++)null!==b.state[e]&&(d=b.indexToCoords(e),d=b.createBlockSprite(64*d.x,64*d.y,c.grid.state[e]),c.blockLayer.addChild(d))}},score:0,gameEnded:!1,update:function(a){this.player.update(a,this);this.player.reachedFinalPosition&&(this.grid.update(a,this),this.player=new b,this.blockLayer.addChild(this.player.node),this.player.hasCollision(this.grid)&&
