...
a._updateChildrenDisplayedOpacity=function(a){cc.Node.prototype.updateDisplayedOpacity.call(a,this._displayedOpacity)};a._updateChildrenDisplayedColor=function(a){cc.Node.prototype.updateDisplayedColor.call(a,this._displayedColor)};a._initBatchTexture=function(){}})();var PrototypeGame=function(){var a="img/background.png img/block0.png img/block1.png img/block2.png img/block3.png img/block4.png img/block5.png".split(" "),c=1,b=function(){var a=this;this.rotation=0;this.blocks=[{x:0,y:1,blockTypeId:Math.floor(6*Math.random())},{x:1,y:1,blockTypeId:Math.floor(6*Math.random())},{x:0,y:0,blockTypeId:-1},{x:1,y:0,blockTypeId:-1}];this.createPlayerBlockSprite=function(a,b){var c=cc.Sprite.create("img/block"+a+".png");c.setAnchorPoint(new cc.Point(0,0));c.x=b%2*50;c.y=
1>=b?50:0;return c};this.rotate=function(){0==a.rotation?(a.rotation=1,a.blocks[3].blockTypeId=a.blocks[1].blockTypeId,a.blocks[1].blockTypeId=a.blocks[0].blockTypeId,a.blocks[0].blockTypeId=-1,a.blocks[2].blockTypeId=-1):1==a.rotation?(a.rotation=2,a.blocks[0].blockTypeId=a.blocks[3].blockTypeId,a.blocks[2].blockTypeId=-1,a.blocks[3].blockTypeId=-1):2==a.rotation?(a.rotation=3,a.blocks[3].blockTypeId=a.blocks[1].blockTypeId,a.blocks[1].blockTypeId=a.blocks[0].blockTypeId,a.blocks[0].blockTypeId=
-1,a.blocks[2].blockTypeId=-1):(a.rotation=0,a.blocks[0].blockTypeId=a.blocks[3].blockTypeId,a.blocks[2].blockTypeId=-1,a.blocks[3].blockTypeId=-1)};this.undoRotation=function(){for(var b=0;3>b;b++)a.rotate()}},d=function(){var a=this;this.elapsedSinceLastUpdate=0;this.structure=new b;this.node=new cc.Node;this.node.setAnchorPoint(new cc.Point(0,-1));this.node.setPosition(50*Math.floor(4.5),500);this.nodeChanged=!0;this.reachedFinalPosition=!1;this.hasCollision=function(b){var c=!1;0>a.node.y&&0!=
a.structure.rotation&&2!=a.structure.rotation&&(c=!0);-64>a.node.y&&(c=!0);if(c)return!0;for(c=0;4>c;c++){var d=a.structure.blocks[c];if(-1<d.blockTypeId){var e=a.getPositionInGrid().x+d.x,d=a.getPositionInGrid().y+d.y;if(b.hasBlockAt(e,d))return!0}}return!1};this.positionWithinBoundaries=function(){0>a.node.x&&1!=a.structure.rotation&&3!=a.structure.rotation&&(a.node.x=0);-50>a.node.x&&(a.node.x=-50);350<a.node.x&&(a.node.x=350)};this.moveBy=function(b,c,d){a.node.x+=50*b;a.node.y+=50*c;a.positionWithinBoundaries();
a.hasCollision(d.grid)&&(a.node.x+=0-50*b,a.node.y+=0-50*c,0>c&&(a.reachedFinalPosition=!0))};this.getPositionInGrid=function(){return{x:a.node.x/50,y:a.node.y/50}};this.rotate=function(b){a.structure.rotate();a.hasCollision(b.grid)?a.structure.undoRotation():(a.nodeChanged=!0,a.positionWithinBoundaries())};this.update=function(b,d){if(a.nodeChanged){a.nodeChanged=!1;a.node.removeAllChildren();for(var e=0;4>e;e++){var f=a.structure.blocks[e].blockTypeId;-1<f&&(f=a.structure.createPlayerBlockSprite(f,
e),a.node.addChild(f))}}(a.elapsedSinceLastUpdate+=b)>=.5*c&&(a.moveBy(0,-1,d),a.elapsedSinceLastUpdate=0)}},e=cc.Scene.extend({blockLayer:{},hudLayer:{},player:{},grid:new function(){this.state=[];for(var a=0;108>a;a++)this.state.push(null);var b=this;this.addPlayerBlocks=function(a){for(var c=0;4>c;c++){var d=a.structure.blocks[c];if(-1<d.blockTypeId){var e=a.getPositionInGrid().x+d.x,f=a.getPositionInGrid().y+d.y,e=b.coordsToIndex(e,f);b.state[e]=d.blockTypeId}}};this.indexToCoords=function(a){return{x:a%
9,y:Math.floor(a/9)}};this.coordsToIndex=function(a,b){return 9*b+a};this.hasBlockAt=function(a,c){return null!==b.state[b.coordsToIndex(a,c)]};this.createBlockSprite=function(a,b,c){c=cc.Sprite.create("img/block"+c+".png");c.setAnchorPoint(new cc.Point(0,0));c.x=a;c.y=b;return c};this.dropFlyingBlocks=function(){var a;for(a=0;a<b.state.length;a++)if(null!==b.state[a]){var c=b.indexToCoords(a);11>c.y&&!b.hasBlockAt(c.x,c.y-1)&&(c=b.coordsToIndex(c.x,c.y-1),b.state[c]=b.state[a],b.state[a]=null,a=
c-1)}};this.removeConnectingBlocks=function(){for(var a=0,c=function(a,d,e){for(var f=b.indexToCoords(a),f=[{x:f.x,y:f.y-1},{x:f.x,y:f.y+1},{x:f.x-1,y:f.y},{x:f.x+1,y:f.y}],g=0;g<f.length;g++){var k=f[g],n=b.coordsToIndex(k.x,k.y);!(0>k.x||8<k.x||0>k.y||11<k.y)&&0<=n&&null!==b.state[n]&&b.state[n]==d&&-1==e.indexOf(n)&&(-1==e.indexOf(a)&&e.push(a),e.push(n),c(n,d,e))}},d=b.state.length-1;0<=d;d--)if(null!==b.state[d]){var e=[];c(d,b.state[d],e);if(5<=e.length){for(var f=0;f<e.length;f++)b.state[e[f]]=
null;a+=e.length}}return a};this.update=function(a,c){b.addPlayerBlocks(c.player);for(var d=null,e=0;0!==d;)e++,b.dropFlyingBlocks(),d=this.removeConnectingBlocks(),c.score+=100*d*e;c.updateHud();c.blockLayer.removeAllChildren();for(e=0;e<b.state.length;e++)null!==b.state[e]&&(d=b.indexToCoords(e),d=b.createBlockSprite(50*d.x,50*d.y,c.grid.state[e]),c.blockLayer.addChild(d))}},score:0,gameEnded:!1,lastSpeedUpdate:0,update:function(a){this.player.update(a,this);this.player.reachedFinalPosition&&(this.grid.update(a,
this),this.player=new d,this.blockLayer.addChild(this.player.node),this.player.hasCollision(this.grid)&&(this.gameEnded=!0));.5<(this.lastSpeedUpdate+=a)&&.3<c&&(this.lastSpeedUpdate=0,c-=.001,this.updateHud());this.gameEnded&&this.unscheduleUpdate()},onEnter:function(){this._super();var a=cc.Sprite.create("img/background.png");a.setAnchorPoint(new cc.Point(0,0));this.addChild(a,0);this.blockLayer=cc.Layer.create();this.blockLayer.setAnchorPoint(new cc.Point(0,0));this.addChild(this.blockLayer,1);
this.hudLayer=cc.Layer.create();this.hudLayer.setAnchorPoint(new cc.Point(0,0));this.hudLayer.setPosition(0,-100);this.addChild(this.hudLayer,2);this.player=new d;this.blockLayer.addChild(this.player.node);cc.eventManager.addListener(f,this);this.updateHud();this.scheduleUpdate()},updateHud:function(){this.hudLayer.removeAllChildren();var a=cc.LabelTTF.create("Connect at least\n5 blocks to\nclear them\n\n\n(use arrow keys\nto move & rotate)\n\n\n\n\n\n\n\n\n\nSCORE:\n\n\n\n\nSPEED:","Arial",20);a.setPosition(550,
420);var b;b=this.score;b="undefined"===typeof b?"000000":("000000"+b).slice(-6);b=cc.LabelTTF.create(b,"Arial",40);b.fillStyle=new cc.Color(185,185,0);b.setAnchorPoint(new cc.Point(0,0));b.setPosition(470,240);var d=100-Math.floor((c-.3)/.7*100),d=cc.LabelTTF.create((100>=d?d:100)+"%","Arial",30);d.fillStyle=new cc.Color(185,185,0);d.setAnchorPoint(new cc.Point(0,0));d.setPosition(470,130);this.hudLayer.addChild(a,0);this.hudLayer.addChild(b,0);this.hudLayer.addChild(d,0)}}),f=cc.EventListener.create({event:cc.EventListener.KEYBOARD,
onKeyReleased:function(a,b){var c=b.getCurrentTarget(),d=c.player;switch(a){case 38:d.rotate(c);break;case 40:d.moveBy(0,-1,c);break;case 37:d.moveBy(-1,0,c);break;case 39:d.moveBy(1,0,c)}}});cc.game.onStart=function(){cc.LoaderScene.preload(a,function(){e();cc.director.runScene(new e)},this);window.focus();document.getElementById("game").focus()};cc.game.run("game")};window.onload=PrototypeGame;
